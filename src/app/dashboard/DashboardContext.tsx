...
export interface CompanyInfo { name: string; phone: string; tz: string; plan: string; }
...
export function DashboardProvider({ children }: { children: React.ReactNode }) {
  ...
  const [profile, setProfile] = useState<UserProfile>({ name: "", email: "", role: "VIEWER", photo: null }); const [company, setCompany] = useState<CompanyInfo>({ name: "", phone: "", tz: "UTC", plan: "STARTER" }); const [plan, setPlan] = useState("STARTER");
  ...
  useEffect(() => { let active = true; (async () => { try {
    ...
    setProfile({ name: [user.firstName, user.lastName].filter(Boolean).join(" "), email: user.email, role: user.role, photo: null }); setCompany({ name: dbCompany.name, phone: "", tz: "UTC", plan: dbCompany.plan }); setPlan(dbCompany.plan);
